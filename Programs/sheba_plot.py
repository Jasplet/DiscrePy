...
class plotter:
    ...
    def plot_by_stat(self,stat,save=False):
        """
        ...
        data = self.data[self.data.STAT==stat]
        ...
        ax1.errorbar(data.BAZ,data.FAST,yerr=data.DFAST,fmt = 'kx',elinewidth=0.5,label='Fast Direction')
        ax1.set_ylim([-90,90])
        ax1.set_ylabel('Fast Direction (deg)')
        ax1.set_xlabel('Back Azimuth (deg)')
        # ax1.legend()

        ax2.errorbar(data.BAZ,data.TLAG,yerr=data.DTLAG,fmt = 'kx',elinewidth=0.5,label='Lag ')
        ax2.set_ylim([0,4])
        ...
        events= ax3.plot(data.EVLO,data.EVLA,'bx',markersize = 5,transform = cart.Geodetic(),label='Event Locations')
        # ax.set_xticks([-130,-125,-120,-115,-110,-105,-100], crs=proj)
        # ax.set_yticks([30,35,40,45,50,55,60], crs=proj)
        stations = ax3.plot(data.STLO[0],data.STLA[0],'kv',transform=cart.Geodetic(),markersize=10,label='Station {}'.format(stat))
        ax3.set_title('SKS coverage for Station {}'.format(stat))
        ax3.legend(bbox_to_anchor=(0.75,0))
